...
def rules_count(board: np.ndarray, x: int, y: int) -> uint8:
    return rules_count_on_rows(board, x, y)+rules_count_on_rows(board.T, x, y)
...
def _generate_game_board(board: np.ndarray) -> np.ndarray:
    ...
    while actual_violations.shape[0]:
        ...
        nearby_min_violations = where_min_violations[np.asarray(distances) == min_distance]
        chosen_cell = np.random.choice(nearby_min_violations)
